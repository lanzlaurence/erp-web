...
export default function Edit({ vendor }: Props) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        city: vendor.city || '',
        suburb_barangay: vendor.suburb_barangay || '',
        ...
        credit_amount: vendor.credit_amount.toString(),
    ...
    useEffect(() => {
        if (vendor.country) {
            const country = countries.find((c) => c.label === vendor.country);
            if (country) {
                ...
                if (vendor.state_province) {
                    const state = stateList.find((s) => s.label === vendor.state_province);
                    if (state) {
                    ...
                    <div className="space-y-4 rounded-lg border p-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-2">
                                <Label>Code</Label>
                                <Input value={vendor.code} disabled className="bg-muted" />
                            </div>
