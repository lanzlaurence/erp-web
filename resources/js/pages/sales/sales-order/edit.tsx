...
import type { SalesOrder } from '@/types/transactions';
...
export default function Edit({ salesOrder, customers, materials, charges }: Props) {
    const { formatAmount } = useFormatters();

    const { data, setData, put, processing, errors } = useForm({
        customer_id:     String(salesOrder.customer_id),
        order_date:      salesOrder.order_date,
        delivery_date:   salesOrder.delivery_date ?? '',
        reference_no:    salesOrder.reference_no ?? '',
        discount_type:   (salesOrder.discount_type ?? '') as 'fixed' | 'percentage' | '',
        ...
        charges: (salesOrder.charges ?? []).map((c) => ({ charge_id: String(c.charge_id) })) as ChargeRow[],
    });
    ...
    const addItem    = () => setData('items', [...data.items, { material_id: '', qty_ordered: '1', unit_price: '0', discount_type: '', discount_amount: '0', unit_price_after_discount: 0, net_price: 0, is_vatable: false, vat_type: 'exclusive', vat_rate: '12', vat_price: 0, gross_price: 0, remarks: '' } as ItemRow]);
    ...
    return (
        <>
            <Head title={`Edit ${salesOrder.code}`} />
            <div className="space-y-6 p-4">
                <div>
                    <h1 className="text-2xl font-semibold">Edit {salesOrder.code}</h1>
                    <p className="text-sm text-muted-foreground">Edit draft sales order</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="space-y-4 rounded-lg border p-6">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <Input value={data.reference_no} onChange={(e) => setData('reference_no', e.target.value)} placeholder="Optional" />
                            </div>
                            {selectedCustomer && (
                            ...
                            <div className="space-y-2">
                                ...
                                <DatePicker value={data.order_date} onValueChange={(val) => { setData('order_date', val); if (data.delivery_date && data.delivery_date < val) setData('delivery_date', ''); }} placeholder="Select order date" />
                                {errors.order_date && <p className="text-sm text-red-600">{errors.order_date}</p>}
                            </div>
                            <div className="space-y-2">
                                <Label>Delivery Date</Label>
                                <DatePicker value={data.delivery_date} onValueChange={(val) => setData('delivery_date', val)} placeholder="Select delivery date" disabled={!data.order_date} minDate={data.order_date ? new Date(data.order_date) : undefined} />
                            </div>
                        </div>
                    </div>

                    {/* Items table — same structure as create */}
                    <div className="space-y-4 rounded-lg border p-6">
                        <div className="flex items-center justify-between">
                            <h3 className="font-semibold">Items</h3>
                            <Button type="button" size="sm" variant="outline" onClick={addItem}><Plus className="mr-2 h-4 w-4" />Add Item</Button>
                        </div>
                                ...
                                <TableBody>
                                    {data.items.map((item, index) => (
                                        <TableRow key={index}>
                                            <TableCell>
                                                <ReactSelect
                                                    ...
                                                    onChange={(opt) => {
                                                        ...
                                                        const updated = [...data.items];
                                                        updated[index] = computeItem({ ...updated[index], material_id: opt?.value ?? '', unit_price: mat ? String(mat.unit_price) : '0' });
                                                        setData('items', updated);
                                                    }}
                                                    placeholder="Select..."
                                                    classNames={selectClass}
                                            ...
                                            <TableCell><InputAmount value={item.qty_ordered} onValueChange={(val) => updateItem(index, 'qty_ordered', String(val ?? 0))} /></TableCell>
                                            <TableCell><InputAmount value={item.unit_price} onValueChange={(val) => updateItem(index, 'unit_price', String(val ?? 0))} /></TableCell>
                                            <TableCell>
                                                <Select value={item.discount_type || 'none'} onValueChange={(val) => updateItem(index, 'discount_type', val === 'none' ? '' : val)}>
                                            ...
                                            <TableCell><InputAmount value={item.discount_amount} onValueChange={(val) => updateItem(index, 'discount_amount', String(val ?? 0))} disabled={!item.discount_type} /></TableCell>
                                            ...
                                            <TableCell><Switch checked={item.is_vatable} onCheckedChange={(val) => updateItem(index, 'is_vatable', val)} /></TableCell>
                                            <TableCell>
                                            ...
                                            <TableCell><Input className="h-8 text-xs w-16" value={item.vat_rate} onChange={(e) => updateItem(index, 'vat_rate', e.target.value)} disabled={!item.is_vatable} /></TableCell>
                                            ...
                                            <TableCell><Input className="h-8 text-xs" value={item.remarks} onChange={(e) => updateItem(index, 'remarks', e.target.value)} placeholder="Optional" /></TableCell>
                                            <TableCell>
                        ...
                        <div className="space-y-6">
                            <div className="space-y-4 rounded-lg border p-6">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        ...
                                        <Select value={data.discount_type || 'none'} onValueChange={(val) => setData('discount_type', val === 'none' ? '' : val as 'fixed' | 'percentage' | '')}>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <InputAmount value={data.discount_amount} onValueChange={(val) => setData('discount_amount', String(val ?? 0))} disabled={!data.discount_type} />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4 rounded-lg border p-6">
                                <div className="flex items-center justify-between">
                                    ...
                                    <Button type="button" size="sm" variant="outline" onClick={addCharge}><Plus className="mr-2 h-4 w-4" />Add Charge</Button>
                                </div>
                                            ...
                                            <div key={index} className="flex items-center gap-3">
                                                <div className="flex-1">
                                                    <ReactSelect
                                                        ...
                                                        onChange={(opt) => { const updated = [...data.charges]; updated[index] = { charge_id: opt?.value ?? '' }; setData('charges', updated); }}
                                                ...
                                                {cr.charge_id && (() => {
                                                    const charge = charges.find((c) => String(c.id) === cr.charge_id);
                                                    return charge ? <div className="text-sm text-muted-foreground whitespace-nowrap"><p>{charge.value_type === 'percentage' ? `${charge.value}%` : formatAmount(Number(charge.value))} — {charge.type}</p></div> : null;
                                                })()}
                                                <Button type="button" variant="ghost" size="sm" onClick={() => removeCharge(index)}><Trash2 className="h-4 w-4 text-red-600" /></Button>
                                            </div>
                                        ))}
                                    </div>
                                )}
                        ...
                        <div className="rounded-lg border p-6 space-y-2 text-sm">
                            ...
                            {[
                                ...
                                <div key={label} className="flex justify-between">
                                    ...
                                    <span className={`font-mono ${value < 0 ? 'text-red-600' : ''}`}>{value < 0 ? '-' : ''}{formatAmount(Math.abs(value))}</span>
                                </div>
                            ))}
                    ...
                    <div className="flex gap-2">
                        <Button type="submit" disabled={processing}>Update Sales Order</Button>
